...
if __name__ == "__main__":
    def test_index():
        print("{:=^100}".format("test_index"))
        spsI = SparseInterval(0, 1)
        spsI.array = [1,2,3,4,5,6]
        x, y = 1.5, 3.5
        print("x = %s, y = %s, x_index = %s, y_index = %s" % (
                x, y, spsI.index(x), spsI.index(y) ) )
        
    def test_add():
        print("{:=^100}".format("test_add"))
        array = [1,2,3,4,5,6]
        spsI = SparseInterval(0, 1)
        spsI.array = array

        """lower内，upper内: [:lower的前前一个] + [lower的前一个 - upper的后一个] + [upper的后后一个:]"""
        print("{:=^60}".format("lower内，upper内"))
        x, y = 1.5, 3.5
        xi, yi = spsI.index(x), spsI.index(y)
        print("x = %s, y = %s, x_index = %s, y_index = %s" % (
            x, y, spsI.index(x), spsI.index(y) ) )
        print(array[:xi-2])
    ...
    def unit_test():
        sps1 = SparseInterval(0, 1)
        sps1.array = [1,2,3,4,5,6]
